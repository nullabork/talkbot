...
function exceeded_daily_limit(server) {
  ...
  var char_count =
    server.dailyStats[server.dailyStats.length - 1].characterCount;
  if (!char_count) return false;
  if (
    !auth.servers ||
    !auth.servers[server.server_id] ||
    !auth.servers[server.server_id].daily_char_limit
  )
    return false;
...
function should_pester(server) {
  ...
  if (
    auth.servers &&
    auth.servers[server.server_id] &&
    auth.servers[server.server_id].pester_threshold
  )
    threshold = auth.servers[server.server_id].pester_threshold;

  return (
    server.stats.characterCount > threshold && threshold > 0 && !server.pestered
  );
}
