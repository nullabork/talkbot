    ...
    if (msg.args[0] * 60000 > 59999) {
      ...
      for (var server_id in msg.world.servers)
        if (msg.world.servers[server_id].inChannel())
          msg.world.servers[server_id].talk(
            "The bot is rebooting in " + msg.args[0] + " minutes"
          );
      setTimeout(function () {
  ...
  for (var id in msg.world.servers) {
    ...
    if (server.isBound()) {
      ...
      if (server.guild.voice.connection)
        chansize = server.guild.voice.connection.channel.members.size;
      active_server_names +=
        server.server_name +
        "(" +
        chansize +
    ...
    if (server.permitted[id]) {
      ...
      if (member) {
        ...
        if (role)
          members +=
            ", (role)" +
            (role.name[0] == "@" ? role.name.substring(1) : role.name);
        else members += ", " + id;
      }
    }
  }
