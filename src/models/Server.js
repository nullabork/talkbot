...
class Server {
    ...
    if (server.inChannel())
      return Common.error(
        "joinVoiceChannel(" +
          voiceChannel.id +
          "): already joined to " +
          server.guild.voiceConnection.channel.id +
          "!"
      );
  ...
  switchVoiceChannel(voiceChannel) {
    ...
    if (!server.guild.voiceConnection)
      return server
        ...
        .catch(Common.error);
    if (voiceChannel.id == server.guild.voiceConnection.channel.id)
    ...
    var settings = {
      ...
      language:
        options.language == "default"
          ? "en-AU"
          : options.language || server.language,
    };
  ...
  stop(reason, all) {
    ...
    if (this.guild.voiceConnection && this.guild.voiceConnection.dispatcher)
      this.guild.voiceConnection.dispatcher.end(reason);
  }
  ...
  playAudioContent(audioContent, format, callback) {
    ...
    var endFunc = (reason) => {
      ...
      if (server.guild.voiceConnection.dispatcher)
        server.guild.voiceConnection.dispatcher.setSpeaking(false);
    ...
    server.voice_timeout = setTimeout(
      () =>
        server.guild.voiceConnection.dispatcher
