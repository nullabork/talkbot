...
(async () => {
  ...
  bot.on("ready", () => {
    ...
    Common.error(
      "LOG CHECKPOINT: THE BOT STARTED. THIS IS NOT AN ERROR MESSAGE"
    );
    world.startup();
  });
  ...
  bot.on("voiceStateUpdate", (oldMember, newMember) => {
    ...
    try {
      ...
      if (
        ...
        (!newMember.voiceChannel || !newMember.voiceChannel.joinable)
      ) {
  ...
  bot.on("guildUnavailable", (guild) =>
    Common.error("guild unavailable: " + guild.id)
  );
